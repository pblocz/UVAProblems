...
int main(int argc,char *argv[]){
    int cases,h,f; bool fcase=true; scanf("%i",&cases);
    while(cases--){ scanf("%i%i",&h,&f);
	if(fcase) fcase=false; else putchar('\n');
	bool fwave=true;
	for(int l=0;l<f;l++){
	    if(fwave) fwave=false; else putchar('\n');
	    for(int i=1;i<=h;i++){
		for(int j=0;j<i;j++) printf("%i",i); putchar('\n'); }
	    for(int i=h-1;i>0;i--){
		for(int j=0;j<i;j++) printf("%i",i); putchar('\n'); }
	}
    }
